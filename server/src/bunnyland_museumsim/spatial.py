...
item being carried, that parent is the holding character, not the room; for an exhibit or a
display case, the parent *is* the museum room. These helpers answer the two questions the
museum logic actually asks:

- ``holder_of(item)`` — who is carrying this item (``None`` if it is loose in a room)?
- ``room_of(entity)`` — which room is this entity ultimately in, whether loose on the floor
  or nested in an inventory or container?
"""
...
def room_of(world: World, entity_id) -> Entity | None:
    ...
    so it works for an item resting in a room *and* one carried in an inventory or a case.
    """
